...
.matrix-wrap { max-height: 82vh; overflow: auto; background: var(--surface);
               border: 1px solid var(--border); border-radius: 10px; padding: 0 10px 10px; }
table.matrix { border-collapse: separate; border-spacing: 2px; }
.matrix th, .matrix td { background: var(--surface); }
.matrix thead th { position: sticky; z-index: 2; }
.matrix thead tr.axes th { top: 0; height: 24px; font-size: 11px; font-weight: 600;
                           color: var(--ink-2); text-align: center; }
/* 軸ごとに1つの箱に見せる。セル間の2pxの隙間がそのまま軸の切れ目になる */
.matrix thead tr.axes th.ax { background: color-mix(in oklab, var(--ink) 6%, var(--surface));
                              border-radius: 6px 6px 0 0; }
.matrix thead tr.inds th { top: 26px; }
.matrix th.muni { position: sticky; left: 0; z-index: 3; text-align: left; font-weight: 400;
                  padding: 0 12px 0 2px; white-space: nowrap; font-size: 12px; }
.matrix thead th.muni { z-index: 4; }
.matrix th.ind { writing-mode: vertical-rl; text-orientation: mixed; height: 168px;
                 vertical-align: bottom; padding: 8px 0; font-weight: 400; font-size: 12px;
                 color: var(--ink-2); white-space: nowrap; }
.matrix th.ind.ref { color: var(--ink-muted); }
/* 軸の変わり目に隙間を空け、上の軸ヘッダの箱と切れ目を合わせる */
.matrix .axis-start { border-left: 6px solid transparent; background-clip: padding-box; }
.matrix tbody tr.group th { font-size: 11px; color: var(--ink-muted); padding-top: 10px; }
.matrix td.cell { width: 26px; height: 18px; border-radius: 3px; text-align: center;
                  font-size: 11px; line-height: 18px; color: var(--ink-2); cursor: default; }
...
.matrix tfoot th { position: sticky; bottom: 0; z-index: 2; font-weight: 400;
                   border-top: 1px solid var(--rule); }
.matrix tfoot th.colsum { font-variant-numeric: tabular-nums; font-size: 11px;
                          color: var(--ink-2); text-align: center; padding-top: 4px; }
.matrix tr.hl th.muni, .matrix .hl-col { outline: 2px solid var(--accent); outline-offset: -1px;
                                          border-radius: 3px; }
...
def render_matrix(
    ...
    col_index = {ind.key: n for n, ind in enumerate(flat)}
    built_count = sum(1 for i in flat if cov[i.key].built)
    # 2軸目以降の先頭列。左に隙間を入れて軸の切れ目を見せる
    starts = {inds[0].key for _, inds in columns if inds} - {flat[0].key}

    def cls(ind: Indicator, *extra: str) -> str:
        return " ".join([*extra, *(["axis-start"] if ind.key in starts else [])])

    # ヘッダ2段（軸 → 指標）
    axes_th = "".join(
        f'<th class="ax" colspan="{len(inds)}">{html.esc(axis.label)}</th>'
        for axis, inds in columns
        if inds
    )
    inds_th = "".join(
        f'<th class="{cls(ind, "ind", "" if ind.include_in_axis else "ref")}"'
        f' data-col="{col_index[ind.key]}">'
    ...
    for code in muni.CODES:
        ...
        if m.region != region:
            region = m.region
            rows.append(
                f'<tr class="group"><th class="muni" colspan="{len(flat) + 2}">{region}</th></tr>'
            )
        cells = []
        for ind in flat:
            c = cov[ind.key]
            st = c.status(code)
            cells.append(
                f'<td class="{cls(ind, "cell", st)}" data-col="{col_index[ind.key]}"'
                f' data-m="{html.esc(m.name)}" data-i="{html.esc(ind.label)}"'
                f' data-s="{STATUS_LABEL[st]}"'
                f' data-v="{html.esc(format_value(c.value(code), ind.unit))}">'
                f"{STATUS_MARK[st]}</td>"
            )
        ok = sum(1 for ind in flat if cov[ind.key].status(code) == OK)
        rows.append(
            f'<tr><th class="muni">{html.esc(m.name)}</th>{"".join(cells)}'
            f'<td class="sum">{ok}/{built_count}</td></tr>'
        )

    foot = "".join(
        f'<th class="{cls(ind, "colsum")}" data-col="{col_index[ind.key]}">'
        f"{cov[ind.key].covered if cov[ind.key].built else '–'}</th>"
        for ind in flat
    )
    ...
    return (
        ...
        f'<thead><tr class="axes"><th class="muni"></th>{axes_th}<th></th></tr>'
        f'<tr class="inds"><th class="muni">自治体</th>{inds_th}'
        '<th class="sum">充足</th></tr></thead>'
        f"<tbody>{''.join(rows)}</tbody>"
        f'<tfoot><tr><th class="muni">値あり /53</th>{foot}<th></th></tr></tfoot>'
        "</table></div>"
        '<p class="note">列の下端は値のある自治体数（分母は53）。行の右端は生成済み指標のうち'
        "値のある数。セルにポインタを合わせると実際の値が出る。</p>"
