...
指標を足すのは、1本では測りきれない理由がある軸だけにしている。数を増やすより、
1本ずつ欠損とスケールの妥当性を確かめられる状態を優先する。
複数本にしてあるのは いきぬき（1本では区部と多摩のどちらかが必ず潰れる）、
しごとば（主指標に12自治体の欠損がある）、つながり（団体の数と活動の量は別物）の3軸。
"""
...
INDICATORS: tuple[Indicator, ...] = (
    ...
    # 軸3: しごとば
    #
    # 分母はどちらも人口ではなく面積。住民が体感する選択肢の数は自宅周辺の施設密度に
    # 比例するため。生件数だと自治体の広さが混ざり、八王子市と武蔵野市がどちらも12件で
    # 同点になる（面積は186km2と11km2で17倍違う）。÷人口だと公園面積比と同じく人口の
    # 少ない自治体が上端に張り付き、1件しかない檜原村が全体2位に来る。
    #
    # 重みはサテライトオフィス0.6・図書館0.4。「作業しに行く場所」そのものを数えて
    # いるのは前者だが、掲載許諾済みの施設しか載らず12自治体が欠損する。図書館は
    # 作業専用の施設ではない代わりに53自治体すべてに1館以上あり、その穴を埋める。
    # 両者の順位相関は0.35で、同じものを測ってはいない。
    Indicator(
    ...
    # 軸5: つながり
    #
    # NPO密度と社会教育事業は等重み。前者は「地域に根を張る団体がいくつあるか」、
    # 後者は「実際に開かれている講座・行事がいくつあるか」で、測っている側面が違う。
    # 順位相関も0.06とほぼ無関係で、どちらを主に置くかを決める根拠が無い。
    # 縮小はどちらにもかける（理由は下記）。
    #
    ...
    # 事業数も同じ理由で縮小が要る。檜原村は年5件で28.6件/万人となり、
    # NPO密度とまったく同じ形で上位に来てしまう（下位40自治体は2〜12件/万人に収まる）。
    Indicator(
        key="social_education_program_count",
        label="社会教育事業",
        axis="community",
        direction="higher_is_better",
        ...
        shrink_denominator=5.0,
        definition=(
            "社会教育の学級・講座・事業の年間開催数 ÷ 人口1万人。"
            "人口の少ない自治体は全体の中央値に寄せて補正。"
        ),
    ),
)
